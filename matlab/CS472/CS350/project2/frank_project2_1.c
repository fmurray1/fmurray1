...
void shortPath(int source, int dest, int numNodes)
{
  ...
  struct edge *curEdge;
  dist[source] = 0;
  comp[source] = 0; //BILLY: mark that we haven't visit source. This will cause the loop below to start at source, which is what we want.
  prev[source] = source;
  int currentNode;
  int new;
  int k;
  ...
  while(done(comp, numNodes) != 1)
    {
      //BILLY: you need to get the vertex with the smallest weight in dist[]
      //printf("entering while loop\n");
      k = -1;
      i = 0;
      for(i; i < numNodes; i++)
	{
	  //BILLY: I don't think you want this condition. It prevents you from finding shorter paths to things you have already visited.
	  if(comp[i] == 0)
	    {
	      //printf("comp[i] == 0\n");
	      ...
	      //printf("set new\n");
	      if(new >= (INT_MAX-1000) && i == (numNodes-1) && min >= (INT_MAX-1000))
		{
